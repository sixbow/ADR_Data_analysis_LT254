function resppathy=NEP_vs_loadingv16A
...
%   method.filter='FFn'                 %Use a file with filename (full path) "FFn" that has cols cm^-1 S21 of the filter, tab delimited 
%                                       % can be done with the scripts in
%                                       ..Generate_arbtrary_Filter_Files
...
KIDstr          = [2];       % only for readallKIDs = 0; indices of KIDs (not names) for example [3 7 9]

%%%%%%% setting for the referebce frequency to get the NEP and efficiency %%%%%%%% 
fref            = 150;       % ref post detection frequency to calulate NEP. 
...
Tc              = 1.35;     % in K, for the alu

%%%%%%%% QO Filter settings %%%%%%%%
method.filter   = '1_6THz';    %Cardiff filter
method.tp       = 'lambda^2' ;
method.pol      = 1; 
% not to be changed %
method.eta_c    = 1 ;%total optical efficiency. =1, DO NOT CHANGE. script determines its value.
method.eta_pb   = 0.5; %0.42 for alu
method.GR       = 4; %4
...
min_resp_power  = 0.3e-15; % Minimum power to fit to (for low bb). Best to set to the max P where the range is linear
...
%             end
            ...
            KIDparam(nKID).ddxdP(m,PBB_n)=KIDparam(nKID).dthdP(m,PBB_n)./(4*KIDparam(nKID).Q(m,PBB_n)); %% ddx/dP
            KIDparam(nKID).ddx2dP(m,PBB_n)=-KIDparam(nKID).dRdP(m,PBB_n)./(4*KIDparam(nKID).Q(m,PBB_n)); %% ddx2/dP, ie radius response devided by Q
        ...
        disp(['*********** Finished with: index= ' num2str(PBB_n) ', Pbb= ' num2str(KIDparam(nKID).Pbbnoise(m,PBB_n)) ', TBB= ' num2str(KIDparam(nKID).Tbbnoise(m,PBB_n)) '***********'])
            ...
            if strcmp(methodPopt,'radius')
                ...
                [~, KIDparam(nKID).Poptindex(PBB_n)]=min(nonzeros(radtemp));
                clear radtemp
            elseif strcmp(methodPopt,'phase')
                phasetemp=KIDparam(nKID).phaseNEPfref(:,PBB_n);phasetemp(phasetemp<0)=1e99;%to catch negative values, can occur where KID is switching in S21
                [~, KIDparam(nKID).Poptindex(PBB_n)]=min(nonzeros(phasetemp));
                clear phasetemp
            end
...
fprintf('NEP_vs_loadingv14A is finished \n')
