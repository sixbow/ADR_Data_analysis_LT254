...
if nargin==0
    ChipInfo.path = ...                                         %Root path containing the measurement data.
        '/Users/jochem/Desktop/lt169/Chip4';
    ...
    ChipInfo.ResonatorInfoFile = ...                            %Name of the file containing resonator information
        [ChipInfo.path '/LT_169chip4.txt'];
    ChipInfo.FilmTc = 1.25;                                    %Transition temperature measured for the film [K].
...
showallplots = 1; %IF 1 the T dependences will be shown in plot for all (KID,P) combinations.
...
TCfrac=1/4.2;      %Fraction of Tc from where the responsivity fit starts. Default: 1/8
...
for p=1:Ntemperatures %Loop over all measured Temperatures
    ...
    if S21data{p}(1,1) ~= 0 %catchong cols of 0's
        ...
        Q(p,:) = NaN;
        S21min(p) = NaN;
        disp(['problem in S21 datafile: T = ' num2str(KID.Temperature(p)) ]);
    end
end
