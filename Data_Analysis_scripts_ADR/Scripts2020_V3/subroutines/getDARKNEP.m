function [NEPR,NEPtheta,NEPcross,Nqp,nqp,NEPGR,tau0,dthetaPdark] = getDARKNEP(dthetadN, dRdN, Stheta, SR, Scross, F, eta_pb, tau, taures, crosslevel, V, Delta, Tc)
% Calculates the experimental dark NEP and other params from the cross PSD
%
% INPUT
% single values unless otherwise specified
% dthetadN, dRdN:   phase and amplitude responsivities per qp (obtained using S21 analysis)%
...
% Nqp               qp number, obtained from cross PSD level
% nqp               qp density (um^-3)
% NEPGR             GR noise limited NEP based upon tau and Nqp (W\rt(Hz))
...
% this code is mathematically ok, but cross level and dR/dN seems
% somegtimes off.
...
tau0 = nqp*2*tau*2*Delta^2/(N0*(kb*Tc)^3);  %tau_0 MUST be realted to the SINGLE particle lifetime; I use 2xtau(experiment) =  tau(single particle)%

% GR limited NEP
NEPGR     = 2*Delta/eta_pb * sqrt(Nqp/(tau));     %tau MUST be  the SINGLE particle lifetime; tau_sp=2xtau exp.%
