...
path        = [ '..' ]; %root path where data is, one higher than the scripts
...
startoptcut     = 4; %   start at BB power 1+ this value for opt eff calc (removes lowest Pbb)
endoptcut       = 0; %   end at end BB power range - this for opt eff calc (removes highest Pbb)
...
maxtau          = 1.2e-3; % maximum realistic lifetime
plottaufit      = 0; % set to 1 only for debugging - creates the tau fit + data, but overplots in its frame
nepplotrangePab    = [1e-20 3e-18];% ylim for all NEP wrt absorbed power figures
nepplotrangePsource    = [0.1e-18 1e-15];% ylim for all NEP wrt source power figures
...
load([resppathy_C 'KIDparam.mat']);clear nepplotrangePabs nepplotrange nepplotrangePs
nepplotrange = nepplotrangePab;
nepplotrangePs = nepplotrangePsource;
...
for nKID=1:nokids
    ...
    KIDparam(nKID).optradeff = 1e-10;%wmean(KIDparam(nKID).radopteffvsP(takenum),1./KIDparam(nKID).stdradopteffvsP(takenum)); %weighted mean of allowed values with 1/errors as weights
    % Error Opt. eff: weighted mean of the error of the optical efficiency radius readout, devided by sqrt(# points) %  
    mean_error =  1e-10;%wmean(KIDparam(nKID).stdradopteffvsP(takenum),1./KIDparam(nKID).stdradopteffvsP(takenum));
   ...
   % Error Opt. eff: weighted mean of the error of the optical efficiency radius readout %  
    ...
    for PBB_nunsorted=1:noBBtoplot
        ...
        loglog(KIDparam(nKID).f_noise{KIDparam(nKID).Poptindex(PBB_n),PBB_n},TrueNEP,'color',colorsPBB(PBB_nunsorted,:),'LineWidth',2)%,'b','LineWidth',1);%xlim([1 5000]);grid on;ylim([10^-19 10^-16]);hold on;
        hold on;
        clear TrueNEP;
        ...
        %Noise spectra
        subplot(2,2,1) %phase
        semilogx(KIDparam(nKID).f_noise{KIDparam(nKID).Poptindex(PBB_n),PBB_n},KIDparam(nKID).phasenoise{KIDparam(nKID).Poptindex(PBB_n),PBB_n},'color',colorsPBB(PBB_nunsorted,:),'LineWidth',2)%,'b','LineWidth',1);%xlim([1 5000]);grid on;ylim([10^-19 10^-16]);hold on;
        hold on;
    ...
    for PBB_nunsorted=1:noBBtoplot
        ...
        subplot(2,2,1)
        semilogx(KIDparam(nKID).f_noise{KIDparam(nKID).Poptindex(PBB_n),PBB_n},KIDparam(nKID).ampnoise{KIDparam(nKID).Poptindex(PBB_n),PBB_n},'--','color',colorsPBB(PBB_nunsorted,:),'LineWidth',2)%,'b','LineWidth',1);%xlim([1 5000]);grid on;ylim([10^-19 10^-16]);hold on;
    ...
    ylim(nepplotrange);grid on;grid minor;
   
    
    %create easy plot ref. PsI{nKID}(minthetaNEPindex)
    [OptNEPmin.theta_Pabs(nKID), minthetaNEPindex] = min(KIDparam(nKID).phaseNEPfref_abs(PsI{nKID}));
    [OptNEPmin.R_Pabs(nKID), minRNEPindex] = min(KIDparam(nKID).radiusNEPfref_abs(PsI{nKID}));
    OptNEPmin.KIDID(nKID) = respkids(nKID);
    temp = KIDparam(nKID).ddxdP(Pindexmatrix);
    OptNEPmin.ddxdP(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    ...
    %raw responsivities (vs Psource). arrays are 2D (Pread, Pbb)
    temp = KIDparam(nKID).dthdP(Pindexmatrix);
    OptNEPmin.dthetadP(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    temp = KIDparam(nKID).dRdP(Pindexmatrix);
    OptNEPmin.RdP(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    %noise levels. Note that the NEP is calcualted directly from the NEP
    %spectra (including the lifetime roll-on). So re-calcuaoting with the
    %noise level could lead to small inaccuracies.
    % NEP = 10.^(KIDparam(nKID).phasenoise{m,PBB_n}/20)/KIDparam(nKID).dthdP(m,PBB_n);%phase NEP
    % 10.^(KIDparam(nKID).phasenoise{m,PBB_n}/20) = phaserootNoiselevelfref
    temp = KIDparam(nKID).phaserootNoiselevelfref(Pindexmatrix);
    OptNEPmin.phaserootNoiselevelfref(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    temp = KIDparam(nKID).radiusrootNoiselevelfref(Pindexmatrix);
    OptNEPmin.radiusrootNoiselevelfref(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    %NEP vs PBB KIDparam(nKID).phaseNEPfref(Pindexmatrix)
    temp = KIDparam(nKID).phaseNEPfref(Pindexmatrix);% = OptNEPmin.theta(nKID)/method.eta_c;
    OptNEPmin.theta_PBB(nKID) = temp(PsI{nKID}(minthetaNEPindex));
    temp =  KIDparam(nKID).radiusNEPfref(Pindexmatrix);%=OptNEPmin.R(nKID)/method.eta_c;
    OptNEPmin.R_PBB(nKID) = temp(PsI{nKID}(minthetaNEPindex));
...
figure(1234)
subplot(1,3,1)
semilogy(OptNEPmin.KIDID,OptNEPmin.theta_Pabs,'ob','MarkerSize',8,'MarkerFaceColor','b');  hold on  
...
subplot(1,3,2)
semilogy(OptNEPmin.KIDID,OptNEPmin.R_Pabs,'ob','MarkerSize',8,'MarkerFaceColor','b');  hold on  
semilogy(OptNEPmin.KIDID,OptNEPmin.R_PBB,'or','MarkerSize',8,'MarkerFaceColor','r');
xlabel('KID ID');ylabel('NEP (W /\surd Hz)');grid on;ylim([1e-20 1e-17]);
title('R readout');legend('Pabs','Psource')
subplot(1,3,3)
semilogy(OptNEPmin.KIDID,OptNEPmin.Efficiency,'ob','MarkerSize',8,'MarkerFaceColor','b');  hold on  
