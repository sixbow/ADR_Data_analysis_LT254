%%
function [fm,SRRraw,SPPraw,SPRraw,nrrejected,sigmaRx5,sigmaPx5] = binfilefunction4(filename,meastime,freq,aver,TDoption, maakdeplots)
...
% SRR: PSD of Radius in dBc/Hz 10*log10(data)
% SPP: PSD of Phase in dBc/Hz
% SPR: cross-PSD of R and P in V^2. (10*log10(SPR) = SPR in dBc/Hz)%
% 
...
while telgood < 2  %this is to catch that all data is filtered, it iteratively increases TDoption.nrsigma until some data gets through  
    ...
    if telgood < 2
        TDoption.nrsigma = TDoption.nrsigma+1;
        Rmmsmooth_reject = zeros(1,rows);Pmmsmooth_reject = Rmmsmooth_reject;
        sigmamodified = 1;
...
if sigmamodified == 1
    disp(['WARNING: Sigma increased to ' num2str(TDoption.nrsigma)]);
    disp('Data likely scared ;-). Rerun with higher sigma')
end
