...
YDATA = (10^(-bs)).*(Model_TLS_lin(xpar,XDATA) + (3*10^(bs-2)).*randn(1,length(XDATA)));
Y = (10^(-bs)).*Model_TLS_lin(xpar,XDATA);

x0 = [1 1];
% Non-linear fit is done in next line!
%options = optimoptions('lsqcurvefit','Algorithm','levenberg-marquardt');
[x,resnorm,~,exitflag,output] = lsqcurvefit(Model_TLS_lin,x0,XDATA,YDATA);%[],[],options);
xcorr = [(10^(bs))*x(1) x(2)];
f1 = figure;
ax1 = axes('XScale','linear','YScale','linear');
hold(ax1,'on')
Y_reconstructed = (10^(-bs)).*Model_TLS_lin(xcorr,XDATA);
