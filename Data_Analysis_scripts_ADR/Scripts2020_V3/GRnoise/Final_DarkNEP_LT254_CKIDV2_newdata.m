...
for kidn=kidn_iter
    ...
    for nT=nT_iter_DataAnalysis 
        legendTvaluesf1{nT} = sprintf('%1.3f',NOISE(kidn).Temperature(nT));
    end
    hTl(kidn) = legend(legendTvaluesf1,'location','eastOutside');
...
hold(ax2, 'off');
title(append('CKID #',string(kidn)));
ylabel('$NEP_{dark}$ ($W/\sqrt{Hz}$)','Interpreter','latex','FontSize',30);
