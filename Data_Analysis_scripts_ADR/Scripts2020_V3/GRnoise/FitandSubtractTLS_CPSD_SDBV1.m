...
options.OptimalityTolerance =  10^(-80);
options.StepTolerance =  10^(-80);
options.FunctionTolerance =  10^(-80);
%% Analysis
close all
for kidn = 1:1%6 %Number of KIDS
    for nT =1:1%4%Number of Temperatures. Likely loop between 1:14  ~length()
        ...
        begin_data_point = 1; 
        end_data_point = 40; % We only want to fit the first part of the TLS noise since there it is dominant
        ...
        % Real part
        Xlog_fitr_Re = log10(Current_freq(begin_data_point:end_data_point)); % f = e^x
        Ylog_fitr_Re = log10(Current_S_CPSD_Re(begin_data_point:end_data_point));
        log_fit_Re = polyfit(Xlog_fitr_Re,Ylog_fitr_Re,1);
        % Imag Part
        Xlog_fitr_Im = log10(Current_freq(begin_data_point:end_data_point)); % f = e^x
        Ylog_fitr_Im = log10(Current_S_CPSD_Im(begin_data_point:end_data_point));
        log_fit_Im = polyfit(Xlog_fitr_Im,Ylog_fitr_Im,1);
        ...
        TLS_corrected_S_CPSD_Re = abs(Current_S_CPSD - LinearModel(log_fit_Re,Current_freq)); %+ ones(1,length(Current_freq))*min(Current_S_CPSD)  ;
        TLS_corrected_S_CPSD_Im = abs(Current_S_CPSD - LinearModel(log_fit_Im,Current_freq)); %+ ones(1,length(Current_freq))*min(Current_S_CPSD)  ;
        ...
        CrossPSDNOISE(IndexP_sub_opt{kidn}(p)).CrossPSD{nT}(:,2) = (CrossPSDNOISE(IndexP_sub_opt{kidn}(p)).CrossPSD{nT}(:,2).*abs(TLS_corrected_S_CPSD_Re))./abs(CrossPSDNOISE(IndexP_sub_opt{kidn}(p)).CrossPSD{nT}(:,2));
