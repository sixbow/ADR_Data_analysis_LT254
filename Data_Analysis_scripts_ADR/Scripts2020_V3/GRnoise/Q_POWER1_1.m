...
close all
clear variables
dpath=[cd '/../../S21_normal/Power'];%Directory of data, no FSP @ end
addpath('../subroutines')
refpowerindex=1;%1=lowest P, 2 is single lowest P etc..
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% qrt matrix info: 
% 1 T                                       
...
for n=1:length(kids)%everyKID
    ...
    for m=1:length(KIDpowers{n}(:))%Every power. Results of all powers will be combined
        ...
        filtered=smooth(data{m}(:,2),20); %smoothing
        ...
        dattofit = data{m}(:,1:3);
        dattofit(:,2) = 10.^((dattofit(:,2))/20);
        [result.Fr,Q,result.Smin,fitevaluation] = FitS21main5(dattofit,1);%[fres,Q,S21min,fitevaluation,errors]
        fitresult{m}(:,1) = fitevaluation(:,1);
        fitresult{m}(:,2) = fitevaluation(:,2);
...
%         
        ...
        qrt(m,10)=result.Smin; % fitted S21min in dB
        qrt(m,4)=Q(2);%Get Qi=Q/S21min
        qrt(m,3)=Q(3);%Get Qc=(Qi-Q/QiQ)
        qrt(m,1)=T;
        qrt(m,13)=Power;
...
%    elseif  j>=4 && qrt(1,13) > qrt(2,13)%do only if there is room to play around optimum power as found before AND power decreasing with filenumber
 ...
 %       disp('check Q_power3_2 for correct optimum finding line 102')
    ...
    for plotbla=1:length(data)
        plot(data{sortindex(plotbla)}(:,1),(data{sortindex(plotbla)}(:,2)),colortabel(plotbla));hold on;%volt S21
    ...
    legend(num2str(legendarray'),'location','best');
    grid on;
    
    subplot(2,2,2)
    semilogy(qrt(:,13),qrt(:,2),'-ok','MarkerSize',4,'MarkerFaceColor','k');
    hold on;semilogy(qrt(:,13),qrt(:,3),'-or','MarkerSize',4,'MarkerFaceColor','r');
    hold on;semilogy(qrt(:,13),qrt(:,4),'-ob','MarkerSize',4,'MarkerFaceColor','b');
    legend('Q','Qc','Qi','location','best');
...
%     hold on; plot(qrt(y,13),[qrt(y,2),qrt(y,3),qrt(y,4)],'-og','MarkerSize',8);
    ...
    legend('Ql','Qc','Qi')  ; grid on  
   
    subplot(2,2,3)
    plot(data{sortindex(jj)}(:,1),(data{sortindex(jj)}(:,2)),'.k','MarkerSize',5);hold on;
    ...
    plot(qrt(:,13),qrt(:,9),'-ok','MarkerSize',3);hold on;grid on;
    legend('Opt. Qi Freq. meth.','Opt Qi','ref.','All data','Location','Best');

    %Write result data
    MakeGoodFigure(14,10,12,[dpath filesep 'powerdep_KID' '_' num2str(kids(n))]);
    ...
    figure(10000)
    subplot(1,2,1)
    semilogy(output(:,9),output(:,3),colortabel1{n},'MarkerFaceColor',colortabel(n));hold on;grid on
    xlabel('Pi [dBm]');ylabel('Q ');
    subplot(1,2,2)
    semilogy(output(:,9),output(:,5),colortabel1{n},'MarkerFaceColor',colortabel(n));hold on;
    xlabel('Pi [dBm]');ylabel('Qi ');
    grid on;
    legstr{n}=['KID ' num2str(kids(n))];
    MakeGoodFigure(10,6,13)
    clear data;
end

legend(legstr,'location','best');
MakeGoodFigure(10,6,13,[dpath filesep 'powerdep_allKIDs']);
