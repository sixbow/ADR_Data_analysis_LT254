...
SW_corr_noise_setup = 0; %Sietse: if 1 corrects setup noise, 0 no correction.
...
for kidn=1:length(KIDnumbers) % LOOP OVER ALL UNIQUE KIDS,
    ...
    %======================================================================
    % Fit all the Pint(noise @ 1 F) data to obtain the noise at Pint = -40 dBm
    %======================================================================
    ...
    Figfile=[Noisepath,'KID',num2str(KIDnumbers(kidn),'%.0f'),'_',...
        num2str(NOISE(IndexPref).Temperature,'%.3g'),'K_NOISE_Pdep1'];
    if SaveStuff == 1
        MakeGoodFigure(9,7,8,Figfile)
    else
        MakeGoodFigure(9,7,8)
    end
...
if ReadPoptfile == 1
   ...
   % ylim([floor(min(minp)) ,ceil(max(maxnp))])
    ...
    Figfile=[Noisepath,'All_KIDS_Pdep',...
