...
%
S21subdir = [ 'Data_LT254_Sietse' filesep 'LT254_Sietse_Chip11' filesep 'S21' filesep '2D_Popt'];
%S21subdir = [ 'Data_LT254_Sietse' filesep 'LT254_Sietse_Chip11' filesep 'S21' filesep 'Power'];
...
TCfrac=1/4.5;      %4.2 seems niceFraction of Tc from where the responsivity fit starts. Default: 1/8
...
ChipInfo.path = ...                                         %Root path containing the measurement data.
    ['..' filesep '..' ];
ChipInfo.ResonatorInfoFile = ...
    [ChipInfo.path filesep 'Data_LT254_Sietse' filesep 'S21input.txt'];    	%Name of the file containing resonator information: 
                                                 	%tab separated
...
%==========================================================================
S21path=[ChipInfo.path filesep S21subdir filesep] %Path containing raw S21 data
...
for n=1:NumberOfS21Files %Loop over all S21 files
    ...
    if n == 1
        ...
        disp('terminate and adjust stopoffset, or continue')
        pause
    end
    format long
    % Assigns:
    ...
    if length(KID(n).Temperature) < 4 %Value 4 is arbitrary. Can be anything >= 2
        ...
        KID(n).dthetadNqpmaxM1 = zeros(1,2);
        KID(n).dRdNqpmaxM1 = zeros(1,2);
        ...
        KID(n).dthetadNqpmaxM1 = max(KID(n).ResponsivityM1(:,1));
        KID(n).dRdNqpmaxM1 = max(KID(n).ResponsivityM1(:,2));
    end
...
if strcmp(ChipInfo.material, 'Al')
    N0=1.72*10^10;%for AL, BEN Mazin p 19 in mu-3 eV-1
...
KID(n).ddxdnqp_um3 = 1e-12*phasefitparam(1)*KID(n).Volume; %ddx/dn, in normalized response /qp/um^3
...
% alpha estimate, valid for Beta = 2;
frequ = KID(n).Fres(1)*1e9*2*pi;
[KID(n).sigma1,KID(n).sigma2,  KID(n).ds1dnqp,KID(n).ds2dnqp, dxdn_alpha_1] = Sigmas_DataAnalysis(frequ, KID(n).N0, KID(n).Delta, KID(n).Temperature(1));

KID(n).alpha = -KID(n).ddxdnqp_um3 / dxdn_alpha_1; %from eqn 2.24: dx/dn = alpha * dx/dn_calcul)alpha=1
%==========================================================================
...
legend([PhaseM1,PhaseM2,AmpM1,AmpM2],'Phase (M1:FIT)','Phase ((M2:NUM)','Amp ((M1:FIT)','Amp (M2:NUM)','location','SouthWest')
hold off
...
%       KID(n).ddxdNqpvalue slope of linear fit to (Fres(T)-Fres(0))/Fres(0) vs Nqp
...
%                                       as a function of temperature as 
%                                       determined by Method 2: numerical
%                                       differentatiopn (unreliable)
